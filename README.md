...
1. 採譜の精度
   - 現状ハイハットクロース、バスドラム、スネア、一部タムの採譜にのみ対応しています
1. ノーツの編集
...
複数のタブの処理を一括で実行したり、複数譜面に対して対して一括で変換かけたりなどもできます

- "0. Workspace"タブ内の"Batch"タブを開きます
- 実行したいタブ名にチェックを入れます
- "Batch Convert Selected Score"を押すと、選択中の譜面に対してバッチ処理を行います
- "Batch Convert All Score"を押すと、ワークスペース中の全譜面に対してバッチ処理を行います
...
バスドラムは曲によってアタックが早かったり遅かったりするので調整が必要です。
