...
with gr.Blocks(title="TubeDTX") as demo:
                    ...
                    with gr.TabItem("Download Video"):
                        with gr.Row():
                            with gr.Column():
                                add_space(1)
                                with gr.Row():
                                    dev_download_button = gr.Button("Download", variant="primary")
                                with gr.Row():
                                    with gr.Column(scale=5):
                                        dev_download_video_output_dir_textbox = gr.Textbox(label="Download Video Output Dir", value=dev_config.download_video_output_dir)
                                    with gr.Column(scale=1, min_width=50):
                                        dev_download_video_output_dir_open_button = gr.Button("Open", variant="primary")
                                dev_download_video_url_textbox = gr.Textbox(label="YouTube URL", value=dev_config.download_video_url)
                            with gr.Column():
                                dev_download_video_output = gr.Textbox(show_label=False)

                    with gr.TabItem("Crop Video"):
                        with gr.Row():
                            with gr.Column():
                                add_space(1)
                                with gr.Row():
                                    dev_crop_video_button = gr.Button("Crop", variant="primary")
                                with gr.Row():
                                    with gr.Column(scale=5):
                                        dev_crop_video_input_path_textbox = gr.Textbox(label="Crop Video Input Path", value=dev_config.crop_video_input_path)
                                    with gr.Column(scale=1, min_width=50):
                                        dev_crop_video_input_path_open_button = gr.Button("Open", variant="primary")
                                    dev_crop_video_start_time_slider = gr.Number(value=dev_config.crop_video_start_time, label="Trim Start Time")
                                    dev_crop_video_end_time_slider = gr.Number(value=dev_config.crop_video_end_time, label="Trim End Time")
                                    dev_crop_video_adjust_start_keyframe_checkbox = gr.Checkbox(value=dev_config.crop_video_adjust_start_keyframe, label="Adjust Start Keyframe")
                            with gr.Column():
    ...
    if dev_config.development:
        dev_config_inputs = [
            ...
            dev_download_video_output_dir_textbox,
            dev_download_video_url_textbox,

            dev_crop_video_input_path_textbox,
    ...
    if dev_config.development:
        ...
        dev_download_video_output_dir_open_button.click(dev_select_download_video_output_dir_gr,
                                    inputs=dev_config_inputs,
                                    outputs=[
                                            dev_download_video_output,
                                            dev_download_video_output_dir_textbox,
                                    ])

        dev_download_button.click(dev_download_video_gr,
                        inputs=dev_config_inputs,
                        outputs=[
                                dev_download_video_output,
                        ])

        dev_crop_video_input_path_open_button.click(dev_select_crop_video_input_path_gr,
                                    inputs=dev_config_inputs,
                                    outputs=[
        ...
        dev_crop_video_button.click(dev_crop_video_gr,
                        inputs=dev_config_inputs,
                        outputs=[
                                dev_crop_video_output,
                        ])
